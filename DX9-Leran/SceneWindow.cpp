...
void SceneWindow::Setup()
{
    ...
    D3DXMatrixTranslation(&matIniWorldTerrain, 0.0f, 0.0f, 0.0f);
    ...
    df.Quality = 0;
    df.PitchAndFamily = 0;
    char *ch = "Times New Roman";
    strcpy_s(df.FaceName, ch); // font style
    if (FAILED(D3DXCreateFontIndirect(pd3dDevice, &df, &Font)))
    {
        throw d3dUtil::ProjectError("D3DXCreateFontIndirect() - FAILED");
    }
}
...
bool SceneWindow::Display(float timeDelta)
{
    if (pd3dDevice)
    {
        ...
        D3DXMATRIX matSkyBoxTran;
        D3DXMatrixTranslation(&matSkyBoxTran, position.x, position.y, position.z);
        ...
        skybox->draw(&matSkyBoxTran);
        terrain->draw(&matIniWorldTerrain);
...
void SceneWindow::cameraControl(float timeDelta, float moveVelocity)
{
...
bool SceneWindow::checkCameraCollision(const D3DXVECTOR3 & cameraPositionNow)
{
    D3DXVECTOR3 position;
    camera.getPosition(&position);
